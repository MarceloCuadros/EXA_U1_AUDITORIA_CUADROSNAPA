...
const App = () => {
  ...
  if (authenticated && dataSource.length === 0) {
    const resultados = activosIniciales.map((activo, index) => {
      const evaluacion = evaluarActivo(activo);
      return {
        key: `${index + 1}`,
        activo: activo.nombre,
        riesgo: evaluacion.riesgo,
        impacto: evaluacion.recomendacion, // puedes usar recomendación aquí
        tratamiento: '-' 
      };
    });
    setDataSource(resultados);
    setCount(resultados.length + 1);
    setSuggestEnabled(true); // habilita el botón de recomendar
  }
}, [authenticated]);
